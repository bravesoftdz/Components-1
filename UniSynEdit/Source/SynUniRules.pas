...
uses
  SynUniFormatNativeXml20, System.Types,
...
var
  ...
  begin
    ...
    if CurrNode.SynToken = nil then
      CanBeToken := False
    else if (CurrNode.BreakType = btTerm) and not CharInSet(fLine[Succ(Run)], CurrRange.fDelimiters) then
      CanBeToken := False
    else
      case CurrNode.SynToken.StartLine of
        slFirstNonSpace:
          for i := 0 to posStart-1 do
            if not CharInSet(fLine[i], [' ', #32, #9]) then begin
              CanBeToken := False;
              Break;
            end;
        slFirst:
...
begin
  ...
  if Assigned(HeadNode) then begin // ≈сли есть дерево токенов на этот символ
    ...
    repeat
      ...
      while (CurrNode.NextNodes.Count > 0) and (fLine[Run] <> #0) do begin
        ...
        if nxtStart = nil then
         if CurrRange.HasNodeAnyStart[CurrRange.CaseFunct(CurrNode.Char)] or
            CharInSet(CurrNode.Char, CurrRange.fDelimiters) or
            CharInSet(CurrRange.CaseFunct(fLine[Run]), CurrRange.fDelimiters) then begin
           nxtStart := CurrNode;
           posNext := Run;
         end;
      end;
      ...
      if CharInSet(fLine[Run], CurrRange.fDelimiters) then begin  // If token can end by delimeter and current symbol is delimeter
        Result := True;            // We've found it!
        ASynToken := prevFind.SynToken;
        Exit;
      end;
...
{begin}
  ...
  for i := 0 to SynSets.Count-1 do begin
    ...
    repeat
      Inc(Run);
    until not CharInSet(fLine[Run], TSynSet(SynSets[i]).CharSet) or (fLine[Run] = #0);
    ...
    if CharInSet(fLine[Run], AllowedDelimiters) then begin
      Result := True;                   // We've found it!
      ASynToken := TSynToken.Create(TSynSet(SynSets[i]).Attributes);
      ASynToken.Temporary := True;
      Exit;
 ...
 begin
  ...
  for i := 0 to fSynTokens.Count-1 do begin       // run all  SynTokens
    ...
    if SynToken.BreakType <> btUnspecified then   // If BreakType defined then
      ...
      if CharInSet(s[Len], fDelimiters) then               //   If last symbol is TermSymbol
  ...
  if fSynSets.Count > 0 then
    for i := 0 to 255 do
      for j := 0 to fSynSets.Count-1 do begin
        if TSynSet(fSynSets[j]).Enabled and CharInSet(Char(i), TSynSet(fSynSets[j]).CharSet) then
...
//          TSetSymbols(SymbolList[Char(i)]).AddSetfSetSymbols;
  ...
  if SymbolList[Char(i)] = nil then
  begin
    if CharInSet(Char(i), fDelimiters) then
