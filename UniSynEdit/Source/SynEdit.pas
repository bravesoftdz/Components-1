{ -------------------------------------------------------------------------------
  ...
  $Id: SynEdit.pas, v. 2.0 2013/23/11 23:28:00 Lasse Rautiainen Exp $
...
uses
  ...
  SynRegExpr, {$IFDEF UNICODE} WideStrUtils, {$ENDIF} Math, SysUtils, Classes;
...
type
  ...
  TSynStatusChange = (scAll, scCaretX, scCaretY, scLeftChar, scTopLine, scInsertMode, scModified, scSelection, scReadOnly);
...
{$ENDIF}
  protected
    ...
    function SB_GetScrollInfo(HWnd: HWnd; BarFlag: Integer;
      var ScrollInfo: TScrollInfo): bool;
    function SB_SetScrollInfo(HWnd: HWnd; BarFlag: Integer;
...
{$ENDIF}
    ...
    procedure NotifyHookedCommandHandlers(AfterProcessing: Boolean; var Command: TSynEditorCommand; var AChar: WideChar;
      Data: pointer); virtual;
    procedure Paint; override;
    procedure PaintGutter(const AClip: TRect; const aFirstRow, aLastRow: Integer); virtual;
    procedure PaintTextLines(AClip: TRect; const aFirstRow, aLastRow, FirstCol, LastCol: Integer); virtual;
  ...
  protected
    ...
    property FontSmoothing: TSynFontSmoothMethod read fFontSmoothing
      write SetFontSmoothing;
  public
...
{$ENDIF}
  ...
  public
    ...
    property LineDivider: TSynLineDivider read fLineDivider
      write SetLineDivider;
    property RightEdge: TSynRightEdge read fRightEdge write SetRightEdge;
    property Background: TSynEditBackground read FBackground
      write SetBackground;
    property LineSpacing: Integer read FLineSpacing write SetLineSpacing;
    property LineSpacingRule: TLineSpacingRule read FLineSpacingRule
      write SetLineSpacingRule;
    property BlockWidth: Integer read fBlockWidth write fBlockWidth default 2;
    property ExpandLines: TSynEditStringList read getExpandLines
      write SetExpandLines;
...
function FlatSB_SetScrollProp(p1: HWnd; Index: Integer; newValue: Integer;
  p4: bool): bool; stdcall; external comctl32 name 'FlatSB_SetScrollProp';
...
function FlatSB_ShowScrollBar(HWnd: HWnd; wBar: Integer; bShow: bool): bool;
  stdcall; external comctl32 name 'FlatSB_ShowScrollBar';
function FlatSB_EnableScrollBar(HWnd: HWnd; wSBflags, wArrows: UINT): bool;
  stdcall; external comctl32 name 'FlatSB_EnableScrollBar';
...
type
  ...
  private
    ...
    {$IFDEF UNICODE} reintroduce; {$ENDIF}
  end;
...
{$ENDIF}
  ...
  if aY >= fLinesInWindow * LineHeight {fTextHeight} then
  begin
    aY := fLinesInWindow * LineHeight {fTextHeight} - 1;
    if aY < 0 then
      aY := 0;
  end;
  Result.Column := Max(1, LeftChar + Round(f));
  Result.Row := Max(1, TopLine + (aY div LineHeight {fTextHeight}));
end;
...
begin
  ...
  Result.Y := (RowCol.Row - fTopLine) * LineHeight {fTextHeight};
{$IFDEF SYN_CLX}
  with ClientRect.TopLeft do
  begin
    Inc(Result.X, X);
...
begin
  ...
  if Y < iScrollBounds.Top then
    fScrollDeltaY := (Y - iScrollBounds.Top) div LineHeight {fTextHeight} - 1
  else if Y >= iScrollBounds.Bottom then
    fScrollDeltaY := (Y - iScrollBounds.Bottom) div LineHeight {fTextHeight} + 1
  else
    fScrollDeltaY := 0;
...
{$ELSE}
  ...
  try
    ...
    Mem := GlobalAlloc(GMEM_MOVEABLE or GMEM_DDESHARE, sizeof(TSynSelectionMode) + SLen + 1);
    if Mem <> 0 then
    begin
      p := GlobalLock(Mem);
      try
...
{$ENDIF}
    ...
    begin
      ...
      if (LastLine >= FirstLine) then
      begin
        rcInval := Rect(0, LineHeight {fTextHeight} * (FirstLine - TopLine), fGutterWidth,
          LineHeight {fTextHeight} * (LastLine - TopLine + 1));
{$IFDEF SYN_CLX}
        with GetClientRect do
          OffsetRect(rcInval, Left, Top);
{$ENDIF}
...
begin
  if Visible and HandleAllocated then
    ...
    begin
      ...
      if (LastLine >= FirstLine) then
      begin
        rcInval := Rect(fGutterWidth, LineHeight {fTextHeight} * (FirstLine - TopLine),
          ClientWidth, LineHeight {fTextHeight} * (LastLine - TopLine + 1));
{$IFDEF SYN_CLX}
        with GetClientRect do
          OffsetRect(rcInval, Left, Top);
{$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      if (eoRightMouseMovesCursor in Options) and
        (SelAvail and not IsPointInSelection
  ...
  begin
    ...
    if bWasSel and (eoDragDropEditing in fOptions) and (X >= fGutterWidth + 2) and (SelectionMode = smNormal) and
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if not(ssAlt in Shift) and FAltEnabled then
          begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              // [rfReplaceAll] ) );
              txt := StringReplace(Lines[RowToLine(p.Row) - 1], #9,
              ...
              begin //
                ...
                for I := FoldRange.CollapsedLines.Count - 1 downto 0 do
                  TSynCompletionProposal(fHint)
                    .ItemList.Add(StringReplace(FoldRange.CollapsedLines[I], #9,
                    StringOfChar(#32, TabWidth), [rfReplaceAll]));
...
begin
  ...
  if (Button = mbRight) and (Shift = [ssRight]) and Assigned(PopupMenu) then
    Exit;
  MouseCapture := False;
  if (sfPossibleGutterClick in fStateFlags) and (X < fGutterWidth) and
    (Button <> mbRight) then
  begin
    DoOnGutterClick(Button, X, Y)
  end
...
begin
  ...
  // else
  CodeFoldingRegion := (X >= (fGutterWidth - 12));

  if CodeFolding.Enabled and CodeFoldingRegion and (Lines.Count > 0) then
  begin
    FoldRange := CollapsableFoldRangeForLine
      (RowToLine(PixelsToRowColumn(X, Y).Row));
...
{$IFDEF SYN_CLX}
  ...
  begin
    ...
    Canvas.FillRect(Bounds(FVScrollBar.Left, FHScrollBar.Top, FVScrollBar.Width,
      FHScrollBar.Height));
  end;
...
{$ENDIF}
  ...
  // lines
  nL1 := Max(TopLine + rcClip.Top div LineHeight {fTextHeight}, TopLine);
  nL2 := MinMax(TopLine + (rcClip.Bottom + LineHeight {fTextHeight} - 1) div LineHeight {fTextHeight}, 1,
    DisplayLineCount);
  ...
  try
    ...
    if (CodeFolding.Enabled) and (Gutter.Visible) and (Lines.Count > 0) then
      ...
      begin
        ...
        xpos := fGutterWidth - 11;

        Inc(xpos);
        Rect.Top := (cLine - fTopLine) * LineHeight;
        Rect.Bottom := Rect.Top + LineHeight;
        Rect.Left := xpos;
        Rect.Right := fGutterWidth - 1;
      ...
      begin
        ...
        with fBookMarkOpt do
          BookmarkImages.Draw(Canvas, LeftMargin + aGutterOff,
            (aMarkRow - TopLine) * LineHeight {fTextHeight}, aMark.ImageIndex);
      ...
      begin
        ...
        begin
          fInternalImage.Draw(Canvas, aMark.ImageIndex, fBookMarkOpt.LeftMargin
            + aGutterOff, (aMarkRow - TopLine) * LineHeight {fTextHeight}, LineHeight {fTextHeight});
        end;
        Inc(aGutterOff, fBookMarkOpt.XOffset);
      end;
    end;
...
{$ENDIF}
  if fGutter.Gradient then
    SynDrawGradient(Canvas, fGutter.GradientStartColor,
      fGutter.GradientEndColor, fGutter.GradientSteps,
      Rect(0, 0, fGutterWidth, ClientHeight), True);
  ...
  begin
    ...
    try
      ...
      rcLine := AClip;
      rcLine.Right := Max(rcLine.Right, fGutterWidth);
...
{$ELSE}
        ...
        begin
          ...
          Windows.ExtTextOut(dc, (fGutterWidth - fGutter.RightOffset - 2) - TextSize.cx,
            rcLine.Top + ((LineHeight - Integer(TextSize.cy)) div 2), 0, @rcLine, PChar(S), Length(S), nil);
          SetBkMode(dc, OPAQUE);
        end
        else
          Windows.ExtTextOut(dc, (fGutterWidth - fGutter.RightOffset - 2) - TextSize.cx,
            rcLine.Top + ((LineHeight - Integer(TextSize.cy)) div 2), ETO_OPAQUE, @rcLine, PChar(S), Length(S), nil);
{$ENDIF}
      end;
  ...
  begin
    Canvas.Brush.Color := fGutter.LeftOffsetColor;
    Canvas.FillRect(Rect(0, 0, fGutter.LeftOffset, ClientHeight));
  end;

  if fGutter.RightOffsetColor <> clNone then
  begin
    Canvas.Brush.Color := fGutter.RightOffsetColor;
    Canvas.FillRect(Rect(fGutterWidth - fGutter.RightOffset, 0, fGutterWidth,
      ClientHeight));
  end;
...
{$ENDIF}
  ...
  if GetWordWrap and fWordWrap.Indicator.Visible then
    for cLine := aFirstRow to aLastRow do
      if LineToRow(RowToLine(cLine)) <> cLine then
        fWordWrap.Indicator.Draw(Canvas, fGutterWidth - Gutter.RightOffset -
...
{$ENDIF}
  ...
  *)
  ...
  begin
    fActiveLine.Indicator.Draw(Canvas, Gutter.LeftOffset + 2,
...
var
  ...
  begin
    ...
    rcLine.Bottom := (aFirstRow - TopLine) * LineHeight {fTextHeight};
    ...
    begin
      ...
      begin
        ...
        rcLine.Top := rcLine.Bottom;
        Inc(rcLine.Bottom, LineHeight {fTextHeight});
        ...
        begin
          ...
          begin
            ...
            FoldRange.HintMarkLeft :=
              ((nTokenPos { + nTokenLen } - 5) * fCharWidth)
            // ###mod pos of [...] to left
            ...
            begin
              ...
              X := rcCollapseMark.Left + fCharWidth - 2;
              Canvas.Rectangle(X, Y, X + 2, Y + 2);
              X := rcCollapseMark.Left + ((fCharWidth * 3) div 2) - 1;
              Canvas.Rectangle(X, Y, X + 2, Y + 2);
              X := rcCollapseMark.Right - fCharWidth;
              Canvas.Rectangle(X, Y, X + 2, Y + 2);
            end;
        ...
        begin
          ...
          for I := 0 to fAllFoldRanges.AllCount - 1 do
            with fAllFoldRanges[I] do
              ...
              begin
                Y := rcLine.Top;
                X := GetLineIndentChars(Lines, FromLine - 1) * fCharWidth;
...
begin
  ...
  begin // column value
    nRightEdge := fTextOffset + fRightEdge.Position * fCharWidth; // pixel value
...
{$ENDIF}
  ...
  rcToken := AClip;
  rcToken.Top := (aLastRow - TopLine + 1) * LineHeight {fTextHeight};
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if fActiveLine.Indicator.Glyph.Empty or not fActiveLine.Indicator.Visible
        then
          r.Left := fGutterWidth;
            ...
            begin
              ...
              begin // Found an open character
                ...
                begin // Otherwise find the end keyword
                  // while (Ptr^ <> #0) and (Ptr^ <> FoldRegions[i].Close^) do       //###mod CF-range in comments
          ...
          rtKeyWord:
            begin
              ...
              begin
                ...
                while (Ptr^ <> #0) and (KeyWordPtr^ <> #0) and
                  (Ptr^ = KeyWordPtr^) do
                begin
                  Inc(Ptr);
                  Inc(KeyWordPtr);
                ...
                begin // Keyword found
                  ...
                  begin
                    repeat // Find the end keyword
                      ...
                      { if KeyWordPtr^ = #0 then begin                                        //###mod CF-range in comments
...
vRect: TRect;
begin
vRect := Rect(0, LineHeight * (FirstRow - TopLine), fGutterWidth,
...
begin
  ...
  begin
    Canvas.CopyRect(Rect(oldPosition, 1, oldPosition + 1, ClientHeight),
      fMoveRightEdgeBitmap.Canvas, Rect(0, 0, 1, ClientHeight));
  end;
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            begin
              TempString := TempString + UnicodeStringOfChar(#32,
                InsertPos - Len - 1) + Str
            end
            else
...
{$IFDEF SYN_CLX}
    begin
      iClip := GetClientRect;
      ScrollWindow(Self, 0, LineHeight {fTextHeight} * Delta, @iClip);
    end
{$ELSE}
    ScrollWindow(Handle, 0, LineHeight {fTextHeight} * Delta, nil, nil)
{$ENDIF}
    else
      Invalidate;
      ...
      begin
        ...
        begin
          if eoScrollPastEol in Options then
            ...
            nMaxScroll :=
              Max(TSynEditStringList(Lines).LengthOfLongestLine, 1);
          ...
          FHScrollBar.LargeChange := CharsInWindow -
            Ord(eoScrollByOneLess in fOptions);
        ...
        begin
          ...
          FVScrollBar.LargeChange := LinesInWindow shr Ord(eoHalfPageScroll
            in fOptions);
...
procedure TCustomSynEdit.KeyString(var S: UnicodeString;
  var Handled: Boolean);
var
  I: Integer;
begin
...
{$IFDEF SYN_COMPILER_18_UP}AnsiStrings.{$ENDIF}StrLCopy(PAnsiChar(Msg.Text), PAnsiChar(AnsiString(Text)), Msg.TextMax - 1);
    Msg.Result :=
    {$IFDEF SYN_COMPILER_18_UP}AnsiStrings.{$ENDIF}StrLen
      (PAnsiChar(Msg.Text));
  end;
end;
...
begin
  if (eoScrollPastEol in Options) and not GetWordWrap then
    ...
    Value.Char := Max(Value.Char, 1);
  Value.Line := MinMax(Value.Line, 1, Lines.Count);
  TempString := Lines[Value.Line - 1] + #0;
          ...
          begin
            ...
            begin
              ...
              if (Length(TmpStr) < CaretX - 1) and
                ...
                TmpStr := TmpStr + UnicodeStringOfChar(#32,
                  CaretX - 1 - Length(TmpStr));
...
function TCustomSynEdit.EventFilter(Sender: QObjectH; Event: QEventH)
  : Boolean;
begin
  ...
  case ct of
    ctHorizontalLine:
      begin
        cw := fCharWidth;
        ch := 2;
        fCaretOffset := Point(0, LineHeight {fTextHeight} - 2);
      end;
    ctHalfBlock:
      begin
        cw := fCharWidth;
        ch := (LineHeight {fTextHeight} - 2) div 2;
        fCaretOffset := Point(0, ch);
      end;
    ctBlock:
      begin
        cw := fCharWidth;
        ch := LineHeight {fTextHeight} - 2;
          ...
          try
            ...
            begin
              ...
              begin
                ...
                SpaceCount2 := 0;
                if (Temp[CaretX - 1] <= #32) and (SpaceCount1 = CaretX - 1)
                then
                begin
                  if eoSmartTabDelete in fOptions then
                  begin
                  ...
                  begin
                    ...
                    while (SpaceCount1 < fTabWidth) and
                      (SpaceCount1 < BackCounter) and (TabBuffer[cx] <> #9) do
                    begin
                      Inc(SpaceCount1);
                      Inc(cx);
                    end;
      ...
      ecChar:
        ...
        begin
          ...
          begin
            ...
            begin
              if (Len > 0) then
                SpaceBuffer := UnicodeStringOfChar(#32,
                  CaretX - Len - Ord(fInserting))
              else
                SpaceBuffer :=
                  GetLeftSpacing(CaretX - Len - Ord(fInserting), True);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (fLastDisplayY <> DisplayY) and fActiveLine.Visible then
        begin
          InvalidateRect(Rect(0, LineHeight * (fLastDisplayY - TopLine),
            ClientWidth, LineHeight * (fLastDisplayY - TopLine + 1)), False);
          InvalidateRect(Rect(0, LineHeight * (DisplayY - TopLine),
            ClientWidth, LineHeight * (DisplayY - TopLine + 1)), False);
        end;
  ...
  begin
    ...
    begin
      while (NewX > DestX) and
        ((NewX - 1 > LineLen) or (Line[NewX - 1] = #32)) do
        Dec(NewX);
    end;
  end;
...
begin
  ...
  begin
    ...
    if GetWordWrap then
      ...
      vMinX := 0;
    while (vLastNonBlank > vMinX) and CharInSet(vText[vLastNonBlank],
      [#32, #9]) do
      Dec(vLastNonBlank);
...
{$IFNDEF UNICODE}
  ...
  begin
    ...
    if Win32PlatformIsUnicode then
      SetWindowLongW(Handle, GWL_WNDPROC,
        Integer(GetWindowLongA(Handle, GWL_WNDPROC)));
  end;
{$ENDIF}
...
{$IFNDEF UNICODE}
  ...
  begin
    ...
    if Win32PlatformIsUnicode then
      SetWindowLongA(Handle, GWL_WNDPROC,
        Integer(GetWindowLongW(Handle, GWL_WNDPROC)));
  end;
{$ENDIF}
{$IFDEF UNICODE}
  // assign WindowText here, otherwise the VCL will call GetText twice
...
var
  rcInval: TRect;
begin
  if (not HandleAllocated) or (Line < 1) or (Line > Lines.Count) or
    (not Visible) then
    Exit;
  ...
  begin
    ...
    rcInval := Rect(fGutterWidth, LineHeight {fTextHeight} * (Line - TopLine),
      ClientWidth, 0);
...
procedure TCustomSynEdit.PluginsAfterPaint(ACanvas: TCanvas;
...
procedure TCustomSynEdit.ProperSetLine(aLine: Integer;
  const ALineText: UnicodeString);
begin
  if eoTrimTrailingSpaces in Options then
