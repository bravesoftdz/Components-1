...
begin
  ...
  begin // versions 1.0 è 1.5  //ShowMessage('Trying load 1.5 version...');
    Result := TSynUniFormatNativeXml15.ImportFromStream(AObject, AStream);
  end
  else
  if Copy(Buffer, 1, 27) = '<UniHighlighter version="1.' then
  begin // version 1.8  //ShowMessage('Trying load 1.8 version...');
    Result := TSynUniFormatNativeXml18.ImportFromStream(AObject, AStream)
  end
  else
  begin // version 2.0  //ShowMessage('Trying load 2.0 version...');
    Result := TSynUniFormatNativeXml20.ImportFromStream(AObject, AStream);
  end;
...
begin
  ...
  if (AObject is TSynUniSyn) then begin
    if TSynUniSyn(AObject).FormatVersion = '1.5' then
      Result := TSynUniFormatNativeXml15.ExportToStream(AObject, AStream)
    else if TSynUniSyn(AObject).FormatVersion = '1.8' then
      Result := TSynUniFormatNativeXml18.ExportToStream(AObject, AStream)
    else
      Result := TSynUniFormatNativeXml20.ExportToStream(AObject, AStream);
  end
  else
