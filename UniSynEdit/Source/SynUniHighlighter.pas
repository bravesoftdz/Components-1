...
type
  ...
  public
    ...
    procedure SetLine(const Value: UnicodeString; LineNumber: Integer); override; { Set current line in SynEdit for highlighting }
...
//----------------------------------------------------------------------------
procedure TSynUniSyn.SetLine(const Value: UnicodeString; LineNumber: Integer);
begin
  if not CurrRange.Prepared then // If current Range isn't ready,
    Prepare(); // then prepare it and its sub-ranges

  fLine := PChar(Value);     // Current string of SynEdit
...
begin
  ...
  if (CurrRange.CloseOnTerm) and CharInSet(fLine[Run], CurrRange.Delimiters) then begin
    if CurrRange.AllowPreviousClose then begin
      CurrRange := CurrRange.Parent;
  ...
  begin // If we can't find token from current position
    ...
    while not (CharInSet(fLine[Run], CurrRange.Delimiters) or CurrRange.HasNodeAnyStart[CurrRange.CaseFunct(fLine[Run])]) do
    begin
