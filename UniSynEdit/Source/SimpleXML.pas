...
procedure TStringBuilder.Add(const s: TXmlString);
var
  anAddLength,
  aNewLength: Integer;
begin
...
begin
	...
	if aPos <> 0 then
		s[aPos] := TXmlChar(FormatSettings.DecimalSeparator);
	Result := StrToFloat(s);
end;
...
type
	...
	TAnsiChars = packed record a, b, c, d: AnsiChar end;
	POctet = ^TOctet;
	TOctet = packed record a, b, c: Byte; end;

procedure OctetToChars(po: POctet; aCount: Integer; pc: PAnsiChars);
var
	o: Integer;
begin
...
procedure CharsToOctet(c: PAnsiChars; o: POctet);
var
	i: Integer;
begin
...
begin
	while not anXml.EOF do begin
		...
		aLine := anXml.FCurLine;
		aCol := anXml.FCurPos;
		if anXml.CurChar = '<' then // символ разметки
			if anXml.Next then
				if anXml.CurChar = '/' then // закрывающий тэг элемента
					Exit
				else if anXml.CurChar = '?' then // инструкция
					ParseProcessingInstruction
				else if anXml.CurChar = '!' then begin
					if anXml.Next then
						if anXml.CurChar = '-' then // коментарий
							ParseComment
						else if anXml.CurChar = '[' then // секция CDATA
							ParseCDATA
						else
							ParseDOCTYPE
				end
