...
begin
  ...
  try
    ...
    S := Buffer;
    if (Length(S) = 0) or S.IsDelimiter('\*?/="<>|:,;+^', 0) then // (StrContainsChar('\*?/="<>|:,;+^', S)) then
    begin
      MessageBeep(MB_ICONHAND);
      if Length(S) > 0 then
        MessageDlg(Format('%s: %s', [SBCControlsFileControlEndEditInvalidName, S]), mtError, [mbOK], 0);
      Exit;
    end;
    ...
    if OldDirName = NewDirName then
      Exit;
    if MessageDlg(Format(SBCControlsFileControlEndEditRename, [ExtractFileName(OldDirName),
    ...
    begin
      ...
      ShowMessage(Format(SBCControlsFileControlEndEditRenameFailed, [OldDirName]));
  finally
    FEdit.Hide;
    FTree.SetFocus;
  end;
